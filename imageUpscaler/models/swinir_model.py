...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3, out_chans=3,
        ...
        # calculate the number of layers in each stage
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.upsample = UpsampleOneStep(upscale, embed_dim, out_chans,
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)  # B L C
        x = x.transpose(1, 2).view(-1, self.embed_dim, x_size[0], x_size[1])  # B C H W

        return x

    def forward(self, x):
        # Input normalization
        if self.img_range == 255:
            x = x / 255.
        ...
        x_body = self.conv_after_body(x_body) + x

        # Upsampling
        if self.upsampler == 'pixelshuffle':
            x = self.conv_before_upsample(x_body)
            x = self.upsample(x)
            x = self.conv_last(x)
        elif self.upsampler == 'pixelshuffledirect':
...
class UpsampleOneStep(nn.Sequential):
    """UpsampleOneStep module (the difference with Upsample is that it always only has 1 conv layer)
